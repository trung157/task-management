...
export class TaskService {
  ...
  private readonly businessRules: TaskBusinessRules = {
    ...
    statusTransitions: {
      ...
      'in_progress': ['completed', 'on_hold', 'review', 'cancelled'],
  ...
  async autoAdjustPriorities(userId: string): Promise<number> {
    try {
      ...
      for (const task of tasks.data) {
        ...
        if (metrics.finalScore >= 20 && task.priority !== 'critical') {
      ...
      for (const task of tasks.data) {
        ...
        switch (task.priority) {
          case 'critical':
            ...
            reason = 'Critical priority suggests immediate completion';
            break;
          case 'high':
            suggestedDate = new Date(baseDate);
  ...
  async getSmartRecommendations(userId: string) {
    try {
      ...
      for (const task of tasks.data) {
        ...
        } else if (task.estimated_hours && task.estimated_hours <= 0.5) {
          recommendations.quickWins.push(task);
        } else if (task.estimated_hours && task.estimated_hours >= 2) {
  ...
  private async handlePostUpdateActions(oldTask: Task, newTask: TaskWithRelations): Promise<void> {
    ...
    // Could add: notification updates, webhook calls, etc.
  }
  ...
  private async handlePostDeletionActions(taskId: string, userId: string): Promise<void> {
    ...
    // Could add: cleanup actions, notification cancellations, etc.
  }
